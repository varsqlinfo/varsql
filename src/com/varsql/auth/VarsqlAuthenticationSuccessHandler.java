...
public class VarsqlAuthenticationSuccessHandler implements
		AuthenticationSuccessHandler {
	protected final Log logger = LogFactory.getLog(this.getClass());

	private RedirectStrategy redirectStrategy = new DefaultRedirectStrategy();

	protected VarsqlAuthenticationSuccessHandler() {
		super();
	}
	...
	protected String determineTargetUrl(final Authentication authentication) {
		...
		for (final GrantedAuthority grantedAuthority : authorities) {
			...
			if (tmpAuthority.equals(Authority.USER.name())) {
				return Authority.USER.mainPage();
			}else if (tmpAuthority.equals(Authority.ADMIN.name())) {
				return Authority.ADMIN.mainPage();
			}else if (tmpAuthority.equals(Authority.MANAGER.name())) {
				return Authority.MANAGER.mainPage();
			}else{
				return Authority.GUEST.mainPage();
			}
		}
