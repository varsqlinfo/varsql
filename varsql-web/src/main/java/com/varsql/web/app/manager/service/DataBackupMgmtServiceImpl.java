...
public class DataBackupMgmtServiceImpl extends AbstractService{
	...
	public ResponseResult dataObjectList(String vconnid) {
		...
		DBConnectionEntity dbConnectionEntity = dbConnectionEntityRepository.findByVconnid(vconnid);

		if(dbConnectionEntity==null){
			resultObject.setResultCode(RequestResultCode.ERROR);
			resultObject.setMessage("connection info not found : "+ vconnid);
			return resultObject; 
		}else{
			DatabaseParamInfo dpi = DatabaseUtils.dbConnectionEntityToDatabaseParamInfo(dbConnectionEntity);
			dpi.setSchema(dbConnectionEntity.getVdbschema());
			dpi.setObjectType(ObjectType.TABLE.getObjectTypeId());

			MetaControlBean dbMetaEnum= MetaControlFactory.getDbInstanceFactory(dbConnectionEntity.getDbTypeDriverProvider().getDbType());
