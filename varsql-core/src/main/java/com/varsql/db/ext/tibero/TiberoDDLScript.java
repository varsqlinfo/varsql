 ...
 * @변경이력 :
 */
public class TiberoDDLScript extends AbstractDDLScript {
	private final Logger logger = LoggerFactory.getLogger(TiberoDDLScript.class);

	public TiberoDDLScript(MetaControlBean dbInstanceFactory){
		super(dbInstanceFactory, DBVenderType.TIBERO);
	}
	...
	public List<DDLInfo> getTables(DatabaseParamInfo dataParamInfo, DDLCreateOption ddlOption, String ...objNmArr) throws Exception {
		...
		for(String name : objNmArr){
			...
			for (int i = 0; i < srcList.size(); i++) {
				...
				ddlStr.append(dataTypeInfo.getJDBCDataTypeMetaInfo().getTypeAndLength(typeName, dataTypeInfo, "", source.getInt(MetaColumnConstants.COLUMN_SIZE), source.getInt(MetaColumnConstants.COLUMN_SIZE), source.getInt(MetaColumnConstants.DECIMAL_DIGITS)));
